...
export default function Sidebar({ collapsed }: { collapsed?: boolean }) {
  ...
  return (
    <div className="flex h-screen w-64 flex-col bg-[var(--color-muted-foreground)] text-[var(--color-muted)]">
      {/* Logo */}
      <div className="px-6 py-6">
        <h1 className="text-xl font-bold tracking-wider text-white text-center">LOGO</h1>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {navSections.map((section) => (
          <div key={section.title} className="mb-6 list">
            <h2 className="mb-3 px-3 text-xs font-semibold uppercase tracking-wider">
              {section.title}
            </h2>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <li key={item.path} className="relative">
                    {isActive && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 h-8 w-1 rounded-r-full bg-[#CFFAFE]" />
                    )}
                    <Link
                      to={item.path}
                      className={cn(
                    ...
                    >
                      <span className={cn(isActive ? "text-[var(--color-secondary-foreground)]" : "text-[var(--color-muted)]")}>
                        {item.icon}
                      </span>
